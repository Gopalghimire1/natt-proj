                            ...
                            {{ $attr->title }}
                                @if (count($attr->submenu))
                                    @foreach ($attr->submenu as $item)
                                        <ul>
                                            <li>{{ $item->title }}
                                                    <a href="{{ route('admin.menu.delete',$item->id)}}" class="badge badge-danger" onclick="return confirm('Are You Sure?');">Del</a>
                                            </li>
                                            @if (count($item->submenu))
                                                @foreach ($item->submenu as $item1)
                                                    <ul>
                                                        <li>{{ $item1->title }}
                                                          <a href="{{ route('admin.menu.delete',$item1->id)}}" class="badge badge-danger" onclick="return confirm('Are You Sure?');">Del</a>
                                                        </li>
                                                    </ul>
                                                @endforeach
                                            @endif
